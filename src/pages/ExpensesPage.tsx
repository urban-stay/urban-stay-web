...
const STYLES = `
  ...
  @media (min-width: 640px) {
    .ex-header-inner { padding: 0 24px; height: 64px; }
  }
  @media (min-width: 1024px) {
    .ex-header-inner { padding: 0 32px; }
  }

  .ex-brand { display: flex; align-items: center; gap: 10px; min-width: 0; }
  .ex-brand-icon {
  ...
  @media (min-width: 480px) {
    .ex-brand-sub { display: block; }
    .ex-brand-title { font-size: 17px; }
  }
  ...
  .ex-btn-gold:hover {
    background: #B06A10;
    box-shadow: 0 5px 20px rgba(196,126,26,0.38);
    transform: translateY(-1px);
  }
  .ex-btn-ghost {
    background: transparent;
  ...
  .ex-btn-ghost:hover { background: var(--surface2); color: var(--text); }
  .ex-btn-danger {
    background: var(--red); color: white; padding: 9px 16px;
    box-shadow: 0 4px 14px rgba(192,57,43,0.25);
  }
  .ex-btn-danger:hover { background: #A93226; transform: translateY(-1px); }

  /* ── Content ── */
  .ex-content { max-width: 1320px; margin: 0 auto; padding: 16px; }
  @media (min-width: 640px)  { .ex-content { padding: 20px 24px; } }
  @media (min-width: 1024px) { .ex-content { padding: 28px 32px; } }
  ...
  @media (min-width: 480px) { .ex-stats { grid-template-columns: 1fr 1fr; gap: 12px; } }
  @media (min-width: 768px) { .ex-stats { grid-template-columns: repeat(3,1fr); gap: 14px; margin-bottom: 18px; } }
  ...
  @media (min-width: 768px) { .ex-stat { padding: 20px 22px; } }
  ...
  .ex-stat::before {
    ...
    opacity: 0.07; transition: opacity 0.2s;
  }
  .ex-stat:hover::before { opacity: 0.14; }
  .ex-stat.st-total::before { background: var(--gold); }
  .ex-stat.st-paid::before  { background: var(--green); }
  ...
  .ex-stat-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
  .ex-stat-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text3); }
  .ex-stat-icon { width: 30px; height: 30px; border-radius: 8px; display: flex; align-items: center; justify-content: center; }
  .si-gold  { background: var(--gold-dim); }
  .si-green { background: var(--green-dim); }
  .si-amber { background: var(--amber-dim); }
  ...
  @media (min-width: 768px) { .ex-stat-val { font-size: 26px; } }
  .sv-gold  { color: var(--gold); }
  .sv-green { color: var(--green); }
  .sv-amber { color: var(--amber); }

  .ex-stat-foot { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
  .ex-badge {
    font-size: 10px; font-weight: 700;
  ...
  /* ── Filters ── */
  .ex-filters { padding: 12px 14px; margin-bottom: 12px; }
  @media (min-width: 640px) { .ex-filters { padding: 14px 18px; margin-bottom: 14px; } }
  ...
  @media (min-width: 480px) { .ex-filters-row { grid-template-columns: 1fr 1fr; } }
  @media (min-width: 768px) { .ex-filters-row { grid-template-columns: 1.6fr 0.9fr 1fr; } }
  @media (min-width: 1024px) { .ex-filters-row { grid-template-columns: 1.6fr 0.9fr 1fr 1.1fr 1.1fr; } }
  ...
  /* ── Desktop Table ── */
  .ex-table-wrap { overflow-x: auto; }
  .ex-table { width: 100%; border-collapse: collapse; min-width: 820px; }
  ...
  .ex-tr:last-child { border-bottom: none; }
  .ex-tr:hover { background: #F7F5F0; }
  .ex-td { padding: 12px 14px; vertical-align: middle; }

  .ex-date-wrap { display: flex; align-items: center; gap: 8px; }
  .ex-date-box {
    width: 30px; height: 30px;
    background: var(--surface2); border: 1px solid var(--border);
    border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;
  }
  .ex-date-main { font-size: 11px; font-weight: 600; color: var(--text); }
  .ex-date-sub  { font-size: 10px; color: var(--text3); margin-top: 1px; }
  .ex-title-main { font-size: 12px; font-weight: 700; color: var(--text); }
  .ex-title-sub  { font-size: 10px; color: var(--text3); margin-top: 2px; font-weight: 400; }
  ...
  .ex-dl-btn:hover { background: rgba(88,166,255,0.18); }

  /* ── Mobile expense cards (< 768px) ── */
  .ex-card-list { display: flex; flex-direction: column; gap: 8px; padding: 10px 12px; }
  .ex-card {
    background: var(--surface2);
    border: 1px solid var(--border);
  ...
  .ex-card-actions { display: flex; gap: 6px; flex-shrink: 0; align-items: center; }
  .ex-card-row { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
  .ex-card-meta { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
  .ex-card-meta-item { display: flex; flex-direction: column; gap: 1px; }
  .ex-card-meta-lbl { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--text3); }
  .ex-card-meta-val { font-size: 11px; font-weight: 600; color: var(--text2); }

  /* ── Empty ── */
  .ex-empty { text-align: center; padding: 48px 24px; }
  @media (min-width: 640px) { .ex-empty { padding: 72px 24px; } }
  .ex-empty-ico {
    width: 52px; height: 52px;
    background: var(--surface2); border: 1px solid var(--border);
  ...
  /* ── Footer ── */
  .ex-tfoot { display: flex; align-items: center; justify-content: space-between; margin-top: 10px; flex-wrap: wrap; gap: 6px; }
  .ex-tfoot-txt { font-size: 11px; color: var(--text3); }
  ...
  @media (min-width: 640px) {
    .ex-overlay { align-items: center; padding: 16px; }
  }
  ...
  .ex-modal {
    ...
    animation: mInMobile 0.28s cubic-bezier(0.34,1.2,0.64,1);
  }
  @media (min-width: 640px) {
    .ex-modal {
      border-radius: 20px;
  ...
  @keyframes mInMobile { from { opacity:0; transform: translateY(100%); } to { opacity:1; transform: translateY(0); } }
  ...
  @media (min-width: 640px) { .ex-mhead { padding: 20px 24px 16px; } }

  .ex-mhead-ico {
    width: 34px; height: 34px;
    background: var(--gold); border-radius: 10px;
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 4px 12px var(--gold-glow); flex-shrink: 0;
  }
  .ex-mtitle {
    font-size: 15px; font-weight: 800;
    color: var(--text); letter-spacing: -0.02em;
  }
  @media (min-width: 640px) { .ex-mtitle { font-size: 16px; } }
  .ex-msub { font-size: 11px; color: var(--text3); margin-top: 2px; }

  /* Form */
  .ex-form { padding: 16px 18px; }
  @media (min-width: 640px) { .ex-form { padding: 22px 24px; } }
  ...
  @media (min-width: 480px) { .ex-grid { grid-template-columns: 1fr 1fr; gap: 14px; } }
  .ex-span { grid-column: 1 / -1; }
  ...
  .ex-field:focus { border-color: var(--gold); box-shadow: 0 0 0 3px var(--gold-dim); background: #fff; }
  .ex-field::placeholder { color: var(--text3); }
  .ex-field option { background: #fff; color: var(--text); }
  ...
  .ex-file-row { display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }
  .ex-file-ico { width: 36px; height: 36px; background: var(--blue-dim); border-radius: 9px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .ex-file-rm {
    width: 30px; height: 30px; background: var(--red-dim);
    border: 1px solid rgba(192,57,43,0.18); border-radius: 8px;
  ...
  /* Delete modal */
  .ex-del-body { padding: 24px 20px; text-align: center; }
  @media (min-width: 640px) { .ex-del-body { padding: 32px 28px; } }
  .ex-del-ico {
    width: 48px; height: 48px; background: var(--red-dim);
    border: 1px solid rgba(192,57,43,0.18); border-radius: 16px;
    display: flex; align-items: center; justify-content: center; margin: 0 auto 14px;
  }
  .ex-del-title { font-size: 17px; font-weight: 800; color: var(--text); letter-spacing: -0.02em; margin-bottom: 8px; }
  .ex-del-desc  { font-size: 12px; color: var(--text2); margin-bottom: 20px; line-height: 1.7; }
  .ex-del-btns  { display: flex; gap: 10px; }
...
const ExpensesPage: React.FC = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const r = await getExpensesAPI();
      if (r?.data) { setExpenses(r.data); setFilteredExpenses(r.data); }
    } catch {}
  };
  ...
  const TH_COLS = ['Date','Expense','Type','Amount','Payment','Status','Bill',...(showActs ? ['Actions'] : [])];

  const downloadBill = (exp: Expense) => {
    if (exp.billUrl) { const a = document.createElement('a'); a.href = exp.billUrl!; a.download = exp.billFileName || `bill_${exp.id}`; document.body.appendChild(a); a.click(); document.body.removeChild(a); }
  };
    ...
    <div className="ex">
      ...
      <header className="ex-header">
        <div className="ex-header-inner">
          ...
          <button className="ex-btn ex-btn-gold" onClick={() => openModal()}>
            <Plus size={13} strokeWidth={3} />
            <span>Add</span>
            <span style={{ display: 'none' }} className="ex-add-full"> Expense</span>
          </button>
        </div>
      </header>
      ...
      <main className="ex-content">
        ...
        <div className="ex-stats">
          <div className="ex-stat st-total a1">
            <div className="ex-stat-header">
              <span className="ex-stat-label">Total Expenses</span>
              <div className="ex-stat-icon si-gold"><Receipt size={14} color="#F0A500" /></div>
            </div>
            <div className="ex-stat-val sv-gold">{fmt(total)}</div>
            <div className="ex-stat-foot">
              <span className="ex-badge eb-gold">{filteredExpenses.length}</span>
              <span className="ex-stat-foot-label">transactions</span>
            </div>
          </div>
          <div className="ex-stat st-paid a2">
            <div className="ex-stat-header">
              <span className="ex-stat-label">Paid</span>
              <div className="ex-stat-icon si-green"><CheckCircle2 size={14} color="#3FB950" /></div>
            </div>
            <div className="ex-stat-val sv-green">{fmt(paid)}</div>
            <div className="ex-stat-foot">
              <span className="ex-badge eb-green">{filteredExpenses.filter(e=>e.status==='Paid').length}</span>
              <span className="ex-stat-foot-label">completed</span>
            </div>
          </div>
          <div className="ex-stat st-pend a3">
            <div className="ex-stat-header">
              <span className="ex-stat-label">Pending</span>
              <div className="ex-stat-icon si-amber"><Clock size={14} color="#F0A500" /></div>
            </div>
            <div className="ex-stat-val sv-amber">{fmt(pending)}</div>
            <div className="ex-stat-foot">
              <span className="ex-badge eb-gold">{filteredExpenses.filter(e=>e.status==='Pending').length}</span>
              <span className="ex-stat-foot-label">awaiting</span>
              {overdue > 0 && <span className="ex-badge eb-red" style={{ marginLeft:'auto' }}>{filteredExpenses.filter(e=>e.status==='Overdue').length} overdue</span>}
            </div>
          </div>
        </div>
        ...
        <div className="ex-panel ex-filters a4">
          <div className="ex-filters-row">
            {/* Search — always first, spans full on xs */}
            <div className="ex-search-wrap" style={{ gridColumn: '1 / -1' }}>
              <Search size={13} className="ex-search-ico" />
              <input className="ex-input ex-search-field" type="text" placeholder="Search expenses, vendors…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
            {[
              { val: String(filterYear), set: (v:string) => setFilterYear(Number(v)), ph:'Year', opts: YEARS.map(y=>[String(y),String(y)]) },
              { val: filterMonth, set: setFilterMonth, ph:'Month', opts: MONTHS.map(m=>[m,m]) },
              { val: filterType, set: setFilterType, ph:'Type', opts: TYPES.map(t=>[t,t]) },
              { val: filterCategory, set: setFilterCategory, ph:'Category', opts: CATS.map(c=>[c,c]) },
            ].map(({ val, set, ph, opts }, i) => (
              <select key={i} value={val} onChange={e => set(e.target.value)} className="ex-input">
        ...
        {/* ══ DESKTOP TABLE (md+) ══ */}
        <div className="ex-panel a5" style={{ display: 'none' }} id="ex-desktop-table">
          <style>{`@media (min-width: 768px) { #ex-desktop-table { display: block !important; } #ex-mobile-cards { display: none !important; } }`}</style>
          <div className="ex-table-wrap">
            <table className="ex-table">
              <thead>
                <tr>{TH_COLS.map(h => <th key={h} className="ex-th">{h}</th>)}</tr>
              ...
              <tbody>
                {filteredExpenses.map(exp => (
                  <tr key={exp.id} className="ex-tr">
                    <td className="ex-td">
                      <div className="ex-date-wrap">
                        <div className="ex-date-box"><Calendar size={12} color="#C0BAB4" /></div>
                        <div>
                          <div className="ex-date-main">{fmtDate(exp.date)}</div>
                          <div className="ex-date-sub">{exp.month} {exp.year}</div>
                        </div>
                    ...
                    <td className="ex-td">
                      <div className="ex-title-main">{exp.title}</div>
                      {exp.vendor && <div className="ex-title-sub">{exp.vendor}</div>}
                      <div style={{ fontSize: 10, color: '#A09A94', marginTop: 2 }}>{exp.category}</div>
                    </td>
                    <td className="ex-td"><span className="ex-type-chip"><Tag size={9} />{exp.expenseType}</span></td>
                    <td className="ex-td"><span className="ex-amt">{fmt(exp.amount)}</span></td>
                    <td className="ex-td"><span className="ex-pay-tag">{exp.paymentMethod}</span></td>
                    <td className="ex-td">
                      <span className={`ex-status ${statusCls(exp.status)}`}>
                        <span className="es-dot" style={{ background: dotClr(exp.status) }} />{exp.status}
                      </span>
                    </td>
                    <td className="ex-td">
                      {exp.billFileName
                        ? <button className="ex-dl-btn" onClick={() => downloadBill(exp)}><Download size={10} />Bill</button>
                        : <span style={{ fontSize: 12, color: '#D0CAC4' }}>—</span>}
                    </td>
                    {showActs && (
                      <td className="ex-td">
                        <div style={{ display:'flex', gap:6 }}>
                          <button onClick={() => openModal(exp)} title="Edit"
                            style={{ width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', background:'#FEF3E2', border:'1px solid #E8C07A', borderRadius:8, cursor: canEdit(exp)?'pointer':'not-allowed', opacity: canEdit(exp)?1:0.4, padding:0 }}>
                            <Edit2 size={13} color="#C47E1A" strokeWidth={2} />
                          </button>
                          <button onClick={() => openDelete(exp)} title="Delete"
                            style={{ width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', background:'#FDECEA', border:'1px solid #E8A09A', borderRadius:8, cursor: canEdit(exp)?'pointer':'not-allowed', opacity: canEdit(exp)?1:0.4, padding:0 }}>
            ...
            {filteredExpenses.length === 0 && (
              <div className="ex-empty">
                <div className="ex-empty-ico"><Receipt size={22} color="#C0BAB4" /></div>
                <div style={{ fontFamily:'Syne,sans-serif', fontSize:15, fontWeight:800, color:'#6B6560', marginBottom:6 }}>No expenses found</div>
                <div style={{ fontSize:12, color:'#A09A94', marginBottom:18 }}>Adjust filters or add a new expense</div>
                <button className="ex-btn ex-btn-gold" style={{ margin:'0 auto' }}
                  onClick={() => { setSearchTerm(''); setFilterYear(new Date().getFullYear()); setFilterMonth(''); setFilterType(''); setFilterCategory(''); }}>
                  Clear Filters
                </button>
              </div>
            )}
        ...
        {/* ══ MOBILE CARDS (< 768px) ══ */}
        <div className="ex-panel a5" id="ex-mobile-cards">
          {filteredExpenses.length === 0 ? (
            <div className="ex-empty">
              <div className="ex-empty-ico"><Receipt size={22} color="#C0BAB4" /></div>
              <div style={{ fontFamily:'Syne,sans-serif', fontSize:15, fontWeight:800, color:'#6B6560', marginBottom:6 }}>No expenses found</div>
            ...
            <div className="ex-card-list">
              {filteredExpenses.map(exp => (
                <div key={exp.id} className="ex-card">
                  {/* Top: title + actions */}
                  <div className="ex-card-top">
                    <div className="ex-card-info">
                      <div className="ex-title-main" style={{ fontSize: 13 }}>{exp.title}</div>
                      {exp.vendor && <div className="ex-title-sub">{exp.vendor}</div>}
                    </div>
                    <div className="ex-card-actions">
                      ...
                      {showActs && (
                        <>
                          <button onClick={() => openModal(exp)} style={{ width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center', background:'#FEF3E2', border:'1px solid #E8C07A', borderRadius:7, cursor: canEdit(exp)?'pointer':'not-allowed', opacity: canEdit(exp)?1:0.4, padding:0 }}>
                            <Edit2 size={12} color="#C47E1A" strokeWidth={2} />
                          </button>
                          <button onClick={() => openDelete(exp)} style={{ width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center', background:'#FDECEA', border:'1px solid #E8A09A', borderRadius:7, cursor: canEdit(exp)?'pointer':'not-allowed', opacity: canEdit(exp)?1:0.4, padding:0 }}>
                            <Trash2 size={12} color="#C0392B" strokeWidth={2} />
                          </button>
                        </>
                      )}
                    </div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:6 }}>
                    <span className="ex-amt" style={{ fontSize: 16 }}>{fmt(exp.amount)}</span>
                    <span className="ex-type-chip"><Tag size={9} />{exp.expenseType}</span>
                  </div>
                  ...
                  <div className="ex-card-meta">
                    <div className="ex-card-meta-item">
                      <span className="ex-card-meta-lbl">Date</span>
                      <span className="ex-card-meta-val">{fmtDate(exp.date)}</span>
                    </div>
                    <div className="ex-card-meta-item">
                      <span className="ex-card-meta-lbl">Payment</span>
                      <span className="ex-card-meta-val">{exp.paymentMethod}</span>
                    </div>
                    <div className="ex-card-meta-item">
                      <span className="ex-card-meta-lbl">Category</span>
                      <span className="ex-card-meta-val">{exp.category}</span>
                    </div>
                    {exp.billFileName && (
                      <div className="ex-card-meta-item">
                        <span className="ex-card-meta-lbl">Bill</span>
                        <button className="ex-dl-btn" style={{ marginTop:2 }} onClick={() => downloadBill(exp)}><Download size={9} />Download</button>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="ex-modal">
            <div className="ex-mhead">
              <div style={{ display:'flex', alignItems:'center', gap:10, minWidth:0 }}>
                <div className="ex-mhead-ico">
                  {editingExpense ? <Edit2 size={14} color="#fff" strokeWidth={2.5} /> : <Plus size={14} color="#fff" strokeWidth={2.5} />}
                </div>
                <div style={{ minWidth:0 }}>
                  <div className="ex-mtitle">{editingExpense ? 'Edit Expense' : 'New Expense'}</div>
                  <div className="ex-msub">{editingExpense ? 'Update existing record' : 'Fill in the details to add'}</div>
              ...
              <button style={{ width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', background:'#FEF3E2', border:'1px solid #E8C07A', borderRadius:8, cursor:'pointer', flexShrink:0, padding:0 }} onClick={closeModal}>
                <X size={13} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="ex-form">
              ...
              <div className="ex-sec-lbl" style={{ marginTop:18 }}>
                <div className="ex-sec-lbl-ico"><Paperclip size={11} color="#F0A500" /></div>
                Bill / Receipt <span style={{ color:'#484F58', textTransform:'none', fontSize:9, fontWeight:500, letterSpacing:0 }}>(Optional)</span>
              </div>
              ...
              <div className="ex-upload">
                {form.billFileName ? (
                  <div className="ex-file-row">
                    <div style={{ display:'flex', alignItems:'center', gap:10, minWidth:0 }}>
                      <div className="ex-file-ico"><FileText size={16} color="#58A6FF" /></div>
                      <div style={{ minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:700, color:'#1A1612', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{form.billFileName}</div>
                        <div style={{ fontSize:10, color:'#A09A94', marginTop:2 }}>{form.billFile ? `${(form.billFile.size/1024).toFixed(1)} KB` : 'Previously uploaded'}</div>
                      </div>
                    </div>
                    <button type="button" className="ex-file-rm" onClick={() => setForm(p=>({...p, billFile:null, billFileName:'', billUrl:''}))}>
                  ...
                  <label style={{ cursor:'pointer', display:'block' }}>
                    <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleFile} style={{ display:'none' }} />
                    <div className="ex-upload-inner">
                      <div className="ex-upload-ico"><Upload size={18} color="#F0A500" /></div>
                      <div style={{ textAlign:'center' }}>
                        <div style={{ fontSize:12, fontWeight:700, color:'#2A2520' }}>Click to upload bill or receipt</div>
                        <div style={{ fontSize:10, color:'#A09A94', marginTop:3 }}>PDF · JPG · PNG · Max 5 MB</div>
                      </div>
      ...
      {isDeleteModalOpen && deletingExpense && (
        <div className="ex-overlay">
          <div className="ex-modal" style={{ maxWidth:380 }}>
            <div className="ex-del-body">
              <div className="ex-del-ico"><Trash2 size={20} color="#F85149" /></div>
              <div className="ex-del-title">Delete Expense?</div>
              <div className="ex-del-desc">
                You're about to permanently delete<br />
                <strong style={{ color:'#1A1612' }}>{deletingExpense.title}</strong>.<br />
