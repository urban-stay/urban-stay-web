...
import { deleteEmployeeByIdAPI, deleteSalaryAPI, getEmployeesAPI, getHistResAPI, getMonthResAPI, markSalaryAPI } from '../service';
import { useUser } from '../context/UserContext';
import { BASE_URL } from '../../config';
...
const SalaryModal: React.FC<{ employee: Employee; currentUser: string; onClose: () => void }> = ({ employee, currentUser, onClose }) => {
  ...
  //   setLoading(true); setError(null);
  //   try {
  //     const [monthRes, histRes] = await Promise.all([getMonthRes(year, month), getHistRes(employee.id)

  //     ]);
  //     if (!monthRes.ok) throw new Error('Failed to load');
  //     const all: SalaryRecord[] = monthRes
  //     const found = all.find(r => r.employeeId === employee.id) ?? null;
  //     setRecord(found);
  //     setNote(found?.note ?? '');
  //     setPaidAmount(found?.paidAmount ?? employee.salary);
                      ...
                      <div key={doc.key} style={{ border: `1.5px dashed ${hasNew ? doc.border : '#e5e7eb'}`, borderRadius: 10, overflow: 'hidden', background: hasNew ? doc.bg : '#fafafa', transition: 'all 0.15s' }}>
                        {!hasNew ? (
                          <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '16px 8px', cursor: 'pointer', minHeight: 100 }}>
                            <input type="file" accept=".jpg,.jpeg,.png,.pdf" style={{ display: 'none' }} onChange={e => handleFileChange(e, doc.key)} />
                            ...
                            <div style={{ textAlign: 'center' }}>
                              <div style={{ fontSize: 11, fontWeight: 600, color: '#374151' }}>{doc.label}</div>
                              {existingUrl
